(ns sports.operation
  "OperationActor -- one sports operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (CoachOps-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Instruction Safety Governor (:govern) and the
  ...
  One graph run = one sports operation (intake -> advise -> govern ->
  ...
  decision to a human operator (licensed educator/coach). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/finalize-certification` ALWAYS reaches this
  node when the governor is clean -- see `sports.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [sports.sportsadvisor :as sportsadvisor]
...
(defn build
  ...
  `sports.store/Store`).
  opts:
    :advisor      -- a `sports.sportsadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (sportsadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CoachOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (sportsadvisor/-advise advisor store request)]
            {:proposal p :audit [(sportsadvisor/trace request p)]})))

      ;; Instruction Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
